...
const tierLabels: Record<string, string> = {
  all: "movo all rides",
  classic: "for Movo Classic",
  premium: "for Movo Premium",
  black: "For Movo Privé Black",
};
...
function AvailableCarsContent() {
  ...
  const headerLabel = tierLabels[tier] ?? "movo all rides";

  return (
    <div className="min-h-screen bg-white flex flex-col" style={{ fontFamily: "var(--font-poppins)" }}>
      {/* Header */}
      <div className="px-4 pt-5 pb-3 border-b border-gray-100">
        <p className="text-[11px] text-gray-400 mb-1">{headerLabel}</p>
        <button
          type="button"
          onClick={() => router.back()}
          className="flex items-center gap-1 text-[13px] text-gray-500 mb-2"
        >
        ...
        <h1 className="text-[20px] font-bold text-gray-900 leading-tight">Available cars for ride</h1>
        <p className="text-[11px] text-gray-400 mt-0.5">{filtered.length} cars found</p>
      </div>
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-bold text-gray-900">{car.name}</p>
                <p className="text-[10px] text-gray-400 mt-0.5">{car.specs}</p>
                <div className="flex items-center gap-1 mt-1">
                  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#2D0A53" strokeWidth="2.5">
                    <path d="M21 10c0 7-9 13-9 13S3 17 3 10a9 9 0 0118 0z" />
                    <circle cx="12" cy="10" r="3" />
                  </svg>
                  <p className="text-[10px] text-gray-500">{car.distance}</p>
                </div>
              </div>
            ...
            <button
              type="button"
              className="w-full py-2 rounded-lg text-white font-bold text-[12px] tracking-widest"
