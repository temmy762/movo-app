...
export async function POST(req: NextRequest) {
  try {
    ...
    if (phone && otp) {
      ...
      await prisma.driver.update({ where: { phone }, data: { password: hashedPassword } });
      await prisma.passwordResetToken.delete({ where: { id: record.id } });
    ...
    await prisma.driver.update({ where: { email: resetToken.email }, data: { password: hashedPassword } });
    await prisma.passwordResetToken.delete({ where: { token } });
